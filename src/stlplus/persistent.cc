...
persistent_illegal_type::persistent_illegal_type(const std::string& type): 
  std::logic_error(std::string("illegal type: ") + type)
{
}

persistent_illegal_type::persistent_illegal_type(unsigned short key): 
  std::logic_error(std::string("illegal key: ") + to_string(key))
{
}

persistent_illegal_type::~persistent_illegal_type(void)
{
}

////////////////////////////////////////////////////////////////////////////////

persistent_dump_failed::persistent_dump_failed(const std::string& message):
  std::runtime_error(std::string("dump failed: ") + message)
{
}

persistent_dump_failed::~persistent_dump_failed(void)
{
}

////////////////////////////////////////////////////////////////////////////////

persistent_restore_failed::persistent_restore_failed(const std::string& message):
  std::runtime_error(std::string("restore failed: ") + message)
{
}

persistent_restore_failed::~persistent_restore_failed(void)
{
}
...
public:
  ...
  dump_context_body(const otext& _device, unsigned char _version):
  ...
  unsigned short lookup_interface(const std::type_info& info) const
    {
      std::string key = info.name();
...
dump_context::dump_context(const otext& _device, unsigned char _version) :
  m_body(0)
{
...
dump_context::callback_data dump_context::lookup_type(const std::type_info& info) const
{
  DEBUG_ASSERT(m_body);
...
public:
  ...
  restore_context_body(const itext& _device) :
  ...
  restore_context::callback_data lookup_type(unsigned short key) const
    {
      callback_map::const_iterator found = m_callbacks.find(key);
      if (found == m_callbacks.end())
        throw persistent_illegal_type(key);
  ...
  const persistent& lookup_interface(unsigned short key) const
    {
      interface_map::const_iterator found = m_interfaces.find(key);
      if (found == m_interfaces.end())
        throw persistent_illegal_type(key);
...
restore_context::restore_context(const itext& _device) :
  m_body(0)
{
...
restore_context::callback_data restore_context::lookup_type(unsigned short key) const
{
  DEBUG_ASSERT(m_body);
...
static void dump_unsigned(dump_context& context, size_t bytes, unsigned char* data)
{
...
static void dump_signed(dump_context& context, size_t bytes, unsigned char* data)
{
...
static void restore_unsigned(restore_context& context, size_t bytes, unsigned char* data)
{
...
static void restore_signed(restore_context& context, size_t bytes, unsigned char* data)
{
  // get the dumped size from the file
  size_t dumped_bytes = (size_t)context.get();
  // restore the dumped bytes but discard any that don't fit
...
void dump(dump_context& context, const bool& data)
{
  context.put((unsigned char)data);
}

void restore(restore_context& context, bool& data)
{
  data = context.get() != 0;
}

// char is dumped and restored as an unsigned char because the signedness of char is not defined and can vary
void dump(dump_context& context, const char& data)
{
  context.put((unsigned char)data);
}

void restore(restore_context& context, char& data)
{
  data = (char)(unsigned char)context.get();
}

void dump(dump_context& context, const signed char& data)
{
  context.put((unsigned char)data);
}

void restore(restore_context& context, signed char& data)
{
  data = (signed char)(unsigned char)context.get();
}

void dump(dump_context& context, const unsigned char& data)
{
  context.put((unsigned char)data);
}

void restore(restore_context& context, unsigned char& data)
{
  data = (signed char)(unsigned char)context.get();
}

void dump(dump_context& context, const short& data)
{
  dump_signed(context, sizeof(short), (unsigned char*)&data);
}

void restore(restore_context& context, short& data)
{
  restore_signed(context, sizeof(short),(unsigned char*)&data);
}

void dump(dump_context& context, const unsigned short& data)
{
  dump_unsigned(context, sizeof(unsigned short), (unsigned char*)&data);
}

void restore(restore_context& context, unsigned short& data)
{
  restore_unsigned(context, sizeof(unsigned short),(unsigned char*)&data);
}

void dump(dump_context& context, const int& data)
{
  dump_signed(context, sizeof(int), (unsigned char*)&data);
}

void restore(restore_context& context, int& data)
{
  restore_signed(context, sizeof(int),(unsigned char*)&data);
}

void dump(dump_context& context, const unsigned& data)
{
  dump_unsigned(context, sizeof(unsigned), (unsigned char*)&data);
}

void restore(restore_context& context, unsigned& data)
{
  restore_unsigned(context, sizeof(unsigned),(unsigned char*)&data);
}

void dump(dump_context& context, const long& data)
{
  dump_signed(context, sizeof(long), (unsigned char*)&data);
}

void restore(restore_context& context, long& data)
{
  restore_signed(context, sizeof(long),(unsigned char*)&data);
}

void dump(dump_context& context, const unsigned long& data)
{
  dump_unsigned(context, sizeof(unsigned long), (unsigned char*)&data);
}

void restore(restore_context& context, unsigned long& data)
{
...
static void dump_float(dump_context& context, size_t bytes, unsigned char* data)
{
  size_t i = bytes;
  // put the size
  context.put((unsigned char)i);
...
static void restore_float(restore_context& context, size_t bytes, unsigned char* data)
{
  // get the dumped size from the file
  size_t dumped_bytes = (size_t)context.get();
...
void dump(dump_context& context, const float& data)
{
  dump_float(context, sizeof(float), (unsigned char*)&data);
}

void restore(restore_context& context, float& data)
{
  restore_float(context, sizeof(float), (unsigned char*)&data);
}

void dump(dump_context& context, const double& data)
{
  dump_float(context, sizeof(double), (unsigned char*)&data);
}

void restore(restore_context& context, double& data)
{
...
void dump(dump_context& context, char*& data)
{
...
void dump(dump_context& context, const std::string& data)
{
  dump_basic_string(context, data);
}

void restore(restore_context& context, std::string& data)
{
