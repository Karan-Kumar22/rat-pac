...
void dump_basic_string(dump_context& context, const std::basic_string<charT,traits,allocator>& data)
{
...
template<typename charT, typename traits, typename allocator>
void restore_basic_string(restore_context& context, std::basic_string<charT,traits,allocator>& data)
{
  data.erase();
  size_t size = 0;
...
template<typename T>
void dump_pointer(dump_context& context, const T* const data)
{
...
template<typename T>
void restore_pointer(restore_context& context, T*& data)
{
  if (data)
  {
    delete data;
...
template<typename T>
void dump_polymorph(dump_context& context, const T* data)
{
  try
  {
    // register the address and get the magic key for it
...
template<typename T>
void dump_interface(dump_context& context, const T* data)
{
  try
  {
    // register the address and get the magic key for it
...
template<typename T>
void dump_complex(dump_context& context, const std::complex<T>& data)
{
...
template<typename T>
void restore_complex(restore_context& context, std::complex<T>& data)
{
  T re, im;
  restore(context,re);
  restore(context,im);
...
template<typename T>
void dump_deque(dump_context& context, const std::deque<T>& data)
{
  dump(context,data.size());
...
template<typename T>
void restore_deque(restore_context& context, std::deque<T>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename T>
void dump_list(dump_context& context, const std::list<T>& data)
{
  dump(context,data.size());
...
template<typename T>
void restore_list(restore_context& context, std::list<T>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
void dump_pair(dump_context& context, const std::pair<K,T>& data)
{
...
template<typename K, typename T>
void restore_pair(restore_context& context, std::pair<K,T>& data)
{
...
void dump_map(dump_context& context, const std::map<K,T,P>& data)
{
  dump(context,data.size());
...
template<typename K, typename T, typename P>
void restore_map(restore_context& context, std::map<K,T,P>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename K, typename T, typename P>
void dump_multimap(dump_context& context, const std::multimap<K,T,P>& data)
{
  dump(context,data.size());
...
template<typename K, typename T, typename P>
void restore_multimap(restore_context& context, std::multimap<K,T,P>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
void dump_set(dump_context& context, const std::set<K,P>& data)
{
  dump(context,data.size());
...
template<typename K, typename P>
void restore_set(restore_context& context, std::set<K,P>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename K, typename P>
void dump_multiset(dump_context& context, const std::multiset<K,P>& data)
{
  dump(context,data.size());
...
template<typename K, typename P>
void restore_multiset(restore_context& context, std::multiset<K,P>& data)
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename T>
void dump_vector(dump_context& context, const std::vector<T>& data)
{
  dump(context,data.size());
  for (size_t i = 0; i < data.size(); i++)
    dump(context,data[i]);
...
template<typename T>
void restore_vector(restore_context& context, std::vector<T>& data)
{
  size_t size = 0;
  restore(context,size);
  data.resize(size);
